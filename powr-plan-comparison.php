<?php
    /**
     * @package POWr Plan Comparison
     * @version 1.6.4
     */
    /*
    Plugin Name: POWr Plan Comparison
    Plugin URI: https://www.powr.io/plugins/plan-comparison
    ...
    */
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////
    ///////////////////////////////////////Create Plan Comparison widget/////////////////////////////////
    class Powr_Plan_Comparison extends WP_Widget{
      public $plugin;
      //Create the widget
      public function __construct(){
        parent::__construct( 'powr_plan_comparison',
                             __( 'POWr Plan Comparison' ),
                             array( 'description' => __( 'Plan Comparison by POWr.io') )
        );
      ...
      public function widget( $args, $instance ){
        ...
        ?>
        <div class='widget powr-plan-comparison' label='<?php echo $label; ?>'></div>
        <?php
      }
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Plan Comparison.
        </p>
        <p>
    ...
    function register_powr_plan_comparison() {
      register_widget( 'Powr_Plan_Comparison' );
    }
    //Use widgets_init action hook to execute custom function
    add_action( 'widgets_init', 'register_powr_plan_comparison' );
    //Create short codes for adding plugins anywhere:
    function powr_plan_comparison_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-plan-comparison' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-plan-comparison' label='$label'></div>";
      }else{
      	"<div class='powr-plan-comparison'></div>";
      }
    }
    add_shortcode( 'powr-plan-comparison', 'powr_plan_comparison_shortcode' );
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_plan_comparison_menu' );
    function powr_plan_comparison_menu() {
      add_menu_page( 'POWr Plan Comparison', 'POWr Plan Comparison', 'manage_options', 'powr-plan-comparison-settings', 'powr_plan_comparison_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_plan_comparison_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/plan-comparison';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/plan-comparison';
      }
    ...
    register_activation_hook(__FILE__, 'powr_plan_comparison_plugin_activate');
      add_action('admin_init', 'powr_plan_comparison_plugin_redirect');

      function powr_plan_comparison_plugin_activate() {
      add_option('powr_plan_comparison_plugin_do_activation_redirect', true);
      }
      ...
      function powr_plan_comparison_plugin_redirect() {
      if (get_option('powr_plan_comparison_plugin_do_activation_redirect', false)) {
          delete_option('powr_plan_comparison_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-plan-comparison-settings&' );
          }
       }
      }
